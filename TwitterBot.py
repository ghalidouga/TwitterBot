...
def debugGui():
    #GET VALUE GUI
    email = userEmailLoginEntry.get()
    password = passwordEntry.get()
    topic2 = topicEntry.get()
    hashtagmode = hashtagCheck.get()
    retweet = retweetCheck.get()
    like = likeCheck.get()
    repeatAction = int(howManyTimesEntry.get())
    delayRandomMin = int(delayMinEntry.get())
    delayRandomMax = int(delayMaxEntry.get())
...
hashtagmodeEntry = Checkbutton(root, text="YES", variable=hashtagCheck)
retweetEntry = Checkbutton(root, text="YES", variable=retweetCheck)
likeEntry = Checkbutton(root, text="YES", variable=likeCheck)
...
hashtagmodeEntry.grid(row=4, column=1)
